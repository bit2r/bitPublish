...
Quarto\cite{quartoarch}를 요약하자면 그림 \ref{fig-quarto}처럼, Quarto 문서를 `knitr` 또는 `jupyter`\index{jupyter}로, 마크다운 문서로 랜더링한 다음 `Pandoc`\index{Pandoc}에 의해 html, PDF 또는 Word 문서를 비롯한 여러 출력 포맷으로 변환해주는 시스템입니다.
...
`caution` 환경을 이용합니다. 정보의 구문은 다음과 같습니다.
...
`idea` 환경을 이용합니다. 정보의 구문은 다음과 같습니다.
...
`shadequote` 환경을 이용합니다. 인용의 구문은 다음과 같습니다.
...
이미지의 페이지 배치는 다음과 같은 `fig-pos`\cite{float} 옵션으로 지정합니다.
...
markdown\index{markdown을}을 이용한 표 생성의 경우에는 캡션이 표 내부에 위치하고, 라벨을 지정할 수 없어서 표를 삽입하는 방법으로 적절하지 않습니다.
...
참고문서 등록은 LaTeX의 `biblatex`\index{biblatex} 패키지를 이용합니다. 이 패키지를 사용할 수 있는 설정은 이미 되어 있으니, 'bitPublish를 이용하여 한글 책 조판하기' 예제를 위한 다음의 \`_quarto.yml'을 참고하여 참고문서 등록을 적용합니다.
...
상기 `YAML` 헤더에서 `chapters` 옵션은 책에 장을 추가하는데 'references.qmd' 파일이 참고문서 등록에 해당하는 파일입니다. 그 내용은 다음과 같습니다. 이 설정은 참고문서를 나타내는 시작 페이지의 제목을 "참고문헌"으로 표기합니다.
...
그리고 참고문서에 등록하기 위해서는 `YAML` 헤더에 `bibliography` 옵션에 기술된 'references.bib' 파일에 참고문서를 정의합니다.
...
\`참고 문헌'이라는 제목의 참고문서에 등록하기 위해서는 본문에 다음과 같이 인용을 위해서 \\cite{} 명령을 사용합니다.

```tex
Quarto 홈페이지의 Quarto에 대한 소개\cite{quarto}는 다음과 같다. 이러쿵 저러쿵...
```

이 본문의 \\cite{} 명령은 '참고 문헌' 페이지에 인용 문서에 대한 정보를 기술하고, 인용된 본문에 링크를 걸어줍니다.
...
그림에 크로스-레퍼런스를 적용해서 그림의 번호 3이 4로 정상적으로 변경되었는데, 조사가 어색합니다. \`그림 2.4과'가 아니라 \`그림 2.4와'로 출력되야하기 때문입니다. 왜냐하면 우리말의 조사에는 다음과 같은 규칙이 있기 때문입니다. 앞단어의 끝소리의 자소에 따라 다음처럼 조사가 결정되기 때문입니다.\cite{kim2007}
