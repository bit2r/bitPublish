...
Quarto[@quartoarch]를 요약하자면 그림 [@fig-quarto]처럼, Quarto 문서를 `knitr` 또는 `jupyter`\index{jupyter}로, `마크다운`(`Markdown`)\index{마크다운}\index{Markdown} 문서로 랜더링한 다음 `팬독`(`Pandoc`)\index{팬독}\index{Pandoc}에 의해 마크다운 문서를 HTML, PDF 또는 Word 문서를 비롯한 여러 출력 포맷으로 변환해주는 시스템입니다.
...
`example` 환경을 이용합니다. 예제를 위한 `example` 환경의 구문은 다음과 같습니다.
...
`예제`를 위해서 다음과 같이 장(chapter)의 헤드에 크로스-레퍼런스를 위한 라벨(label)을 정의해야 합니다.
...
크로스-레퍼런스가 정상적으로 동작하기 위해서는 라벨을 정의할 때, 반드시 접두어 **sec-**을 사용해야 합니다. 
:::

한 장에 여러 예제를 만들수 있습니다. 그래서 개별 예제를 식별할 수 있도록 예제에 순번을 매겨야 합니다. 순번을 생성하기 위해서 카운터를 설정합니다.
...
\begin{example}{\ref{sec-bitpublish}.\arabic{exam_num_bitpub}}
\examplelabel{ex1}{\ref{sec-bitpublish}.\arabic{exam_num_bitpub}}
...
```{=tex}
예제 \ref{ex1}를 레퍼런스하기 위해서는 레퍼런스 명령 $\backslash$ref\{\}를 사용합니다.
```
...
그런데 R 플롯에 한글이 포함될 경우에 [@fig-hist]처럼 한글이 출력되지 않는 문제가 발생합니다.
...
그런 후, R 플롯을 그리는 `knitr`\index{knitr} 청크에 `nanum` 옵션의 값을 true로 추가합니다. 이 설정은 `나눔스퀘어` 폰트를 사용해서 R 플롯을 시각화한다는 것을 의미합니다. 다음 스크립트는 [@fig-hist2]\과 같은 R 그래픽스 플롯을 출력합니다.
